...
def model_run(dataset,model_name,gpu_id,num_fea,output,batch=128,**kwargs):
    ...
    final_df.loc[len(final_df)] = [dataset,model_name,test_mse,test_mae,test_rmse]
    final_df.to_excel(f'results/final_df_{dataset}_{model_name}.xlsx', index=False)
...
if __name__ == "__main__":
    for dataset in ['total_stop']:
        print(dataset)

        t1 = MyThread(model_run, args=(dataset, 'NO_CONTEXT_HCOATT', 0, 3, 'fm'))
        t2 = MyThread(model_run, args=(dataset, 'NO_TEXT_HCOATT', 6, 3, 'fm'))
        t3 = MyThread(model_run, args=(dataset, 'No_WORD_HCOATT', 7, 3, 'fm'))
