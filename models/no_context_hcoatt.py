...
class NO_CONTEXT_HCOATT(nn.Module):
    '''
    NARRE: WWW 2018
    '''
    def __init__(self, opt):
        super(NO_CONTEXT_HCOATT, self).__init__()
        self.opt = opt
    ...
    def forward(self, datas):
        ...
        for i in range(self.head):
            ...
            # --------word_co-attention --------------------
            user_word_att, item_word_att = w_coatt(user_word_embedding, item_word_embedding)
            ...
            user_fea = self.dropout(torch.sum(user_att_review_embedding, 1))  # [batch, review_hidden_size]
            item_fea = self.dropout(torch.sum(item_att_review_embedding, 1)) # [batch, review_hidden_size]
    ...
    def reset_para(self):
        for fc in [self.user_fc_layer, self.item_fc_layer,self.u_fc,self.i_fc]:
