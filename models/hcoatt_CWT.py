...
class HCOATT_CWT(nn.Module):
    '''
    NARRE: WWW 2018
    '''

    def __init__(self, opt):
        super(HCOATT_CWT, self).__init__()
        self.opt = opt
        ...
        self.user_id_embedding = nn.Embedding(self.opt.user_num, self.opt.word_dim)  # user/item num * 32
        self.item_id_embedding = nn.Embedding(self.opt.item_num, self.opt.word_dim)  # user/item num * 32
        self.travel_type_embedding = nn.Embedding(self.opt.travel_type_num, self.opt.word_dim)
        self.travel_month_embedding = nn.Embedding(self.opt.travel_month_num, self.opt.word_dim)
        ...
        self.fea_coatt = Co_Attention(dim=self.opt.word_dim, gumbel=False, pooling='avg')
        self.fea_fc_layer = nn.Linear(self.opt.word_dim, self.opt.id_emb_size)

        self.word_coatt = Co_Attention(dim=self.review_hidden_size, gumbel=False, pooling='avg')
        self.review_coatt = Co_Attention(dim=self.opt.id_emb_size, gumbel=False, pooling='avg')

        # self.word_coatt = nn.ModuleList([Co_Attention(dim=self.review_hidden_size, gumbel=False, pooling='avg') for _ in range(self.head)])
    ...
    def forward(self, datas):
        ...
        word_embedding = torch.concat((user_word_embedding, item_word_embedding), 1)
        word_att, fea_att = self.fea_coatt(word_embedding, feature_embedding)

        fea_att_embedding = feature_embedding * fea_att
        fea_att_embedding = self.fea_fc_layer(fea_att_embedding)
        ...
        word_att_embedding = word_embedding * word_att
        u_fea_words, i_fea_words = torch.split(word_att_embedding, [self.opt.u_max_r*self.opt.r_max_len, self.opt.i_max_r*self.opt.r_max_len], 1)


        # --------word_co-attention --------------------
        user_word_att, item_word_att = self.word_coatt(u_fea_words, i_fea_words)
        ...
        user_review_embedding = self.dropout(
            self.relu(self.user_fc_layer(torch.sum(user_att_word_embedding, 2))))  # [batch, review_hidden_size]
        item_review_embedding = self.dropout(
            self.relu(self.item_fc_layer(torch.sum(item_att_word_embedding, 2))))  # [batch, review_hidden_size]

        # --------review_co-attention --------------------
        user_review_att, item_review_att = self.review_coatt(user_review_embedding, item_review_embedding)
        ...
        # user_2att_review_embedding, item_2att_review_embedding = torch.split(review_att_embedding,
        #                                                                      [self.opt.u_max_r, self.opt.i_max_r], 1)
        user_fea = torch.sum(user_att_review_embedding, 1)  # [batch, review_hidden_size]
        item_fea = torch.sum(item_att_review_embedding, 1)  # [batch, review_hidden_size]

        user_fea = torch.stack([user_id_emb, travel_type_emd, user_fea], 1)
        item_fea = torch.stack([item_id_emb, travel_month_emd, item_fea], 1)

        return user_fea, item_fea

    def reset_para(self):
        for fc in [self.fea_fc_layer,self.user_fc_layer, self.item_fc_layer]:
