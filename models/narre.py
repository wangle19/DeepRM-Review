...
class Net(nn.Module):
    ...
    def forward(self, reviews, ids, ids_list):
        ...
        # ------------------linear attention-------------------------------
        re_lin = self.review_linear(fea)
        f_u_i =F.relu(u_i_id_emb)
        id_lin = self.id_linear(F.relu(u_i_id_emb))
        ...
        fc_r_fea=self.fc_layer(r_fea)
        out_a = torch.stack([id_emb, self.fc_layer(r_fea)], 1)
