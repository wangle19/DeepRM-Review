...
def numerize(data):
    ...
    travel_type=list(map(lambda x: travel_type2id[x], data['travel_type']))
    travel_month = list(map(lambda x: travel_month2id[x], data['travel_month']))
    data['user_id'] = uid
    data['item_id'] = iid
    data['travel_type'] = travel_type
    data['travel_month'] = travel_month
    return data
...
if __name__ == '__main__':
    ...
    # for data_file in ['TripAdvisor_500000.csv','TripAdvisor_1000000.csv','TripAdvisor_3000000.csv']:

    data_file = 'weight_5.csv'  #total_stop
    word_dim = 300
    ...
    if len(data.columns) == 9:
        data.columns = ['user_id', 'item_id', 'rating', 'reviews', 'review_time', 'helpful', 'travel_month','travel_year', 'travel_type']
    else:
    ...
    data = data[['user_id', 'item_id', 'rating', 'reviews', 'travel_type', 'travel_month', 'travel_year']]
    # data=data[0:400]
    data = data.astype(str)
    type_num = data['travel_type'].unique()
    print("type_num",type_num)

    uidList, iidList = get_count(data, 'user_id'), get_count(data, 'item_id')
    travel_type_List, travel_month_List= get_count(data, 'travel_type'),get_count(data, 'travel_month')
    userNum_all = len(uidList)
    itemNum_all = len(iidList)
    travel_typeNum_all = len(travel_type_List)
    travel_monthNum_all = len(travel_month_List)
    print("===============Start:all  rawData size======================")
    print(f"dataNum: {data.shape[0]}")
    print(f"userNum: {userNum_all}")
    print(f"itemNum: {itemNum_all}")
    print(f"travel_typeNum: {travel_typeNum_all}")
    print(f"travel_monthNum: {travel_monthNum_all}")
    ...
    travel_type2id = dict((travel_type, i) for (i, travel_type) in enumerate(travel_type_List))
    travel_month2id = dict((travel_month, i) for (i, travel_month) in enumerate(travel_month_List))
    data = numerize(data)
    ...
    for train_index, test_index in kf.split(data):
        ...
        dataset_name = f'{data_file[:-4]}_{cv}'
        print(dataset_name)
        ...
        def extract(data_dict):
            ...
            for i in data_dict.values:
                ...
                travel_type = i[4]
                travel_month = i[5]
                x.append([uid, iid,travel_type,travel_month])
                y.append(float(i[2]))
            return x, y
        ...
        para_df = pd.DataFrame(columns=['vocab_size','word_dim','r_max_len','u_max_r','i_max_r','train_data_size',
                                        'test_data_size','val_data_size','user_num','item_num'])
        ...
        para_dic['travel_type_num'] = travel_typeNum_all
        para_dic['travel_month_num'] = travel_monthNum_all

        # # 保存
        f = open('../config/' + dataset_name + '_para_dict_rating.txt', 'w')
